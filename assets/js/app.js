...
(function () {
  ...
  /* ---------- 联系方式 ---------- */
  const qq = p.contacts?.qq || "";
  $("contacts").innerHTML = `
    <div class="contact-item contact-item--qr contact-item--qq">
      ${p.qqQr ? `<div class="contact-item__qr">
        <img src="${esc(p.qqQr)}" alt="QQ 二维码" loading="lazy" decoding="async" />
        <span>QQ：${esc(qq)}</span>
      </div>` : ""}
    </div>
    ${p.bilibiliQr ? `<div class="contact-item contact-item--bili">
      <div class="contact-item__qr">
        <img src="${esc(p.bilibiliQr)}" alt="B 站二维码" loading="lazy" decoding="async" />
        <span>扫码关注 B 站</span>
      </div>
    </div>` : ""}`;
    ...
    <div class="price-cat">
      ...
      ${(cat.groups || []).map(g => `
        <div class="price-sub">
          ...
          ${g.note ? `<div class="price-sub__note price-sub__note--before">${esc(g.note)}</div>` : ""}
          ${g.afterNote ? `<div class="price-sub__note price-sub__note--after">${esc(g.afterNote)}</div>` : ""}
        </div>`).join("")}
  ...
  function workPlayer(w) {
    ...
    const tabs = hasN ? `
      <div class="player-tabs">
        ...
        ${hasB ? `<button class="player-tab" data-src="bilibili">▶ B站视频</button>` : ""}
      </div>` : hasB ? `
      <div class="player-tabs">
        <button class="player-tab active" data-src="bilibili">▶ B站视频</button>
      </div>` : "";
  ...
  document.addEventListener("click", (e) => {
    ...
    wrap.querySelectorAll(".player-tab").forEach(t => t.classList.toggle("active", t.dataset.src === src));
  ...
  function renderWorks() {
    ...
    $("worksGrid").innerHTML = list.map(w => {
      ...
      const coverStyle = banner
        ? `style="background-image:url('${esc(banner)}');background-position:${esc(coverPos)};background-size:${esc(coverSize)}"`
        : "";
      const tags = [
      ...
      return `<div class="work-card">
        ...
        <div class="work-card__body">
          ...
          <div class="work-card__vocal${w.vocal ? "" : " work-card__vocal--empty"}"><span class="vocal-label">演唱</span>${w.vocal ? esc(w.vocal) : "&nbsp;"}</div>
          ${workPlayer(w)}
        </div>
      </div>`;
    }).join("");
